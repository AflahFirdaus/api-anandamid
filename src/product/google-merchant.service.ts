...
export class GoogleMerchantService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (!rawJson || !this.merchantId) {
      this.logger.warn('Service dinonaktifkan — GOOGLE_SERVICE_ACCOUNT_JSON atau GOOGLE_MERCHANT_ID belum diset di .env');
      return;
    }
    ...
    try {
      ...
      this.logger.error('Gagal parse credentials JSON:', err.message);
    }
  }
  ...
  async syncProduct(product: any): Promise<any> {
    ...
    try {
      ...
      this.logger.error('Sync error:', error.response?.data?.error?.message || error.message);
      throw error;
    }
  }
  ...
  async deleteProduct(productId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Delete error:', error.response?.data?.error?.message || error.message);
    }
  }
