...
export class ProductImportService {
  ...
  async generateTemplate(): Promise<Buffer> {
    ...
    const exampleRow2_Var1 = [
      ...
      13,
      7,
      5, // Berat 120 gram, PxLxT dalam cm
      true,
      true,
      'https://example.com/mouse-merah-depan.jpg',
      'https://example.com/mouse-merah-samping.jpg',
      '',
      '',
      '',
      '',
    ...
    const finalSockets =
      sockets.length > 0 ? sockets : ['LGA 1700', 'AM4', 'AM5'];
    const finalRams = rams.length > 0 ? rams : ['DDR4', 'DDR5'];

    const socketSheet = XLSX.utils.aoa_to_sheet([
      ['socket_type'],
      ...finalSockets.map((s) => [s]),
    ]);
    const ramSheet = XLSX.utils.aoa_to_sheet([
      ['ram_type'],
      ...finalRams.map((r) => [r]),
    ]);
  ...
  async uploadProducts(buffer: Buffer) {
    ...
    const brandMap = new Map(
      dbBrands.map((b) => [b.name.trim().toLowerCase(), b]),
    );
    ...
    for (const [key, group] of groupedProducts.entries()) {
      ...
      try {
        if (!mainRow.category_code)
          throw new BadRequestException(
            `Category code wajib diisi untuk produk '${mainRow.name}'`,
          );

        const category = await this.categoryRepo.findOne({
          where: { code: mainRow.category_code },
        });
        if (!category)
          throw new BadRequestException(
            `Category code ${mainRow.category_code} tidak ditemukan`,
          );

        let productBrand: Brand | null = null;
        if (mainRow.brand_name) {
          productBrand =
            brandMap.get(String(mainRow.brand_name).trim().toLowerCase()) ||
            null;
          if (!productBrand)
            throw new BadRequestException(
              `Brand '${mainRow.brand_name}' tidak ditemukan`,
            );
        }
        ...
        product.variant_type_name = mainRow.variant_type_name
          ? String(mainRow.variant_type_name).trim()
          : null;
        product.socket_type = mainRow.socket_type
          ? String(mainRow.socket_type).trim()
          : null;
        product.ram_type = mainRow.ram_type
          ? String(mainRow.ram_type).trim()
          : null;

        product.is_active =
          mainRow.is_active === true || mainRow.is_active === 'true';
        product.is_popular =
  ...
  // ==========================
  async generateUpdateTemplate(
    categoryCodes?: string[],
    onlyWithSku: boolean = true,
  ): Promise<Buffer> {
    ...
    products.forEach((product) => {
      ...
      const variants =
        product.variants && product.variants.length > 0
          ...
          : [{} as any];
      const hasVariants =
        product.variant_type_name && product.variant_type_name.trim() !== '';
      ...
      variants.forEach((variant, index) => {
        ...
        const rowData = [
          ...
          isFirst ? product.variant_type_name || '' : '',
          variant.variant_name || 'Default',
          variant.price_normal || 0,
          variant.price_discount || 0,
          variant.stock || 0,
          clean(variant.sku_seller),
          isFirst ? product.warranty : '',
        ...
        if (includeHardwareCols)
          rowData.push(
            isFirst ? product.socket_type || '' : '',
            isFirst ? product.ram_type || '' : '',
          );
        ...
        rowData.push(
          isFirst ? product.is_active : '',
          isFirst ? product.is_popular : '',
          ...rowImages,
        );
        rows.push(rowData);
      });
    ...
    for (let R = 1; R <= range.e.r; ++R) {
      for (let C = 0; C <= range.e.c; ++C) {
        ...
        worksheet[cellAddress] = {
          ...
          s: {
            ...(cell.s || {}),
            fill: isEmpty ? { fgColor: { rgb: 'FFFFCC' } } : undefined,
            border: borderStyle,
          },
        };
      }
  ...
  async updateProducts(buffer: Buffer) {
    ...
    const categoryMap = new Map(
      categories.map((c) => [String(c.code).trim(), c]),
    );

    const dbBrands = await this.brandRepo.find();
    const brandMap = new Map(
      dbBrands.map((b) => [String(b.name).trim().toLowerCase(), b]),
    );
    ...
    for (const row of rows) {
      if (!row.id) continue;
      if (!groupedProducts.has(row.id))
        groupedProducts.set(row.id, { mainRow: row, variantsRows: [] });
    ...
    for (const [productId, group] of groupedProducts.entries()) {
      ...
      try {
        ...
        const excelCategoryCode = mainRow.category_code
          ? normalize(mainRow.category_code)
          : null;
        if (excelCategoryCode) {
          const category = categoryMap.get(excelCategoryCode);
          if (!category)
            throw new BadRequestException(
              `Category code ${excelCategoryCode} tidak ditemukan`,
            );
          product.category = category;
        }

        const excelBrandName = mainRow.brand_name
          ? normalize(mainRow.brand_name).toLowerCase()
          : null;
        if (excelBrandName) {
        ...
        if (mainRow.variant_type_name !== '')
          product.variant_type_name = String(mainRow.variant_type_name).trim();
        if (mainRow.socket_type !== '')
          product.socket_type = mainRow.socket_type
            ? String(mainRow.socket_type).trim()
            : null;
        if (mainRow.ram_type !== '')
          product.ram_type = mainRow.ram_type
            ? String(mainRow.ram_type).trim()
            : null;
        if (mainRow.is_active !== '')
          product.is_active =
            mainRow.is_active === true || mainRow.is_active === 'true';
        if (mainRow.is_popular !== '')
          product.is_popular =
            mainRow.is_popular === true || mainRow.is_popular === 'true';
        ...
        for (const vRow of variantsRows) {
          ...
          let variantData: any =
            variantId && currentVariantsMap.has(variantId)
              ? currentVariantsMap.get(variantId)
              : new ProductVariant();

          if (vRow.variant_name !== '')
            variantData.variant_name = String(vRow.variant_name).trim();
          if (vRow.price_normal !== '')
            variantData.price_normal = Number(vRow.price_normal);
          if (vRow.price_discount !== '')
            variantData.price_discount = Number(vRow.price_discount);
        ...
        if (!hasVariants) {
          ...
          for (let i = 1; i <= 10; i++) {
            ...
            if (url && String(url).trim() !== '')
              newImageUrls.push(String(url).trim());
          }
          ...
          for (const newUrl of newImageUrls) {
            ...
            if (matchedIdx !== -1) {
              ...
              try {
                ...
                this.logger.error(`Gagal memproses gambar baru ${newUrl}`, err);
              }
            }
          }
          ...
          const oldVariantImages = existingImages.filter(
            (img) => img.variant_id,
          );
          ...
          for (let vIndex = 0; vIndex < variantsRows.length; vIndex++) {
            ...
            let varImages = existingImages.filter(
              (img) => img.variant_id === variantData.id,
            );
            const newImageUrls: string[] = []; // 🔥 FIX TS2345
            ...
            let sortOrder = 0;
            for (const newUrl of newImageUrls) {
              const matchedIdx = varImages.findIndex(
                (img) => img.image_url === newUrl,
              );
              ...
              if (matchedIdx !== -1) {
                ...
                try {
                  const processed =
                    await this.productService.processSingleImage(
                      newUrl,
                      sortOrder,
                    );
                  if (processed) {
                  ...
                  this.logger.error(
                    `Gagal memproses gambar variasi ${newUrl}`,
                    err,
                  );
                }
              }
